...
SYSTEM_PROMPT       = (
    ...
    "Avoid markdown, bullet points, or special characters since your output will be spoken aloud."
)
...
# Whisper STT
WHISPER_MODEL_SIZE  = os.environ.get("WHISPER_MODEL", "base.en")  # tiny.en|base.en|small.en|medium.en
WHISPER_DEVICE      = "cpu"
...
def stream_llm(messages: List[Dict], api_key: str, model: str) -> Iterator[str]:
    ...
    Runs the HTTP fetch in a daemon thread so the per-read socket timeout
    in requests cannot cause an uninterruptible hang in the caller.
    """
...
def main() -> None:
    ...
    print(f"Loading Whisper ({WHISPER_MODEL_SIZE}) ...", flush=True)
    ...
    while True:
        try:
            ...
            user_text = transcribe(audio, whisper)
            if not user_text:
